...
vec3 triangle::get_trianglepoint0() {
	return trianglepoint0;
}

vec3 triangle::get_trianglepoint1() {
	return trianglepoint1;
}

vec3 triangle::get_trianglepoint2() {
	return trianglepoint2;
}

std::vector<float> triangle::get_material() {
	return material;
}
...
float hit_triangle(int trianglenumber,std:: vector<triangle> trianglepoint, vec3 eyeposition, vec3 screenpoint) {
	...
	vec3 p1 = trianglepoint[trianglenumber].get_trianglepoint0(), p2 = trianglepoint[trianglenumber].get_trianglepoint1(), p3 = trianglepoint[trianglenumber].get_trianglepoint2();
...
vec3 trianglecolor(const vec3 screenpoint,const vec3 eyeposition, int trianglenumber, vec3 light, float t, std::vector<triangle> trianglepoint, std::vector<sphere> sphere,int reflecttime){
	...
	vec3 p1 = trianglepoint[trianglenumber].get_trianglepoint0(), p2 = trianglepoint[trianglenumber].get_trianglepoint1(), p3 = trianglepoint[trianglenumber].get_trianglepoint2();
	vector<float> material = trianglepoint[trianglenumber].get_material();
	float I;
	...
	if (reflecttime > 0 && material[7] != 0) {
		...
		if (t < 0) {
			...
			if (t >= 0) {
			reflect = material[7] * trianglecolor(normalpoint + viewdirection + 2 * normalvector, normalpoint, trianglenumber, light, t, trianglepoint, sphere, reflecttime - 1);
			}
		}
		if (t < 0) {
...
std::vector<float> sphere::get_material() {
	return material;
}
...
float hit_sphere(int spherenumber,std::vector<sphere> sphere, vec3 eyeposition, vec3 screenpoint) {
	...
	vec3 center = sphere[spherenumber].get_center();
	float radius = sphere[spherenumber].get_radius();
...
vec3 spherecolor(vec3 screenpoint, vec3 eyeposition, int spherenumber, std::vector<sphere> sphere, vec3 light, float t, std::vector<triangle> trianglepoint, int reflecttime) {
	...
	vec3 rgb,reflect(0, 0, 0), shadow(0, 0, 0);
	float I;
	vector<float> material = sphere[spherenumber].get_material();
	vec3 spherecenter = sphere[spherenumber].get_center();
	float radius = sphere[spherenumber].get_radius();
	viewdirection = (screenpoint - eyeposition).normalize();
