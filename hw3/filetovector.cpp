...
std::vector<vec3> file_screen(std::vector<std::vector<std::string>>& data2) {
	float angle = 90;
	vec3 eye, view_direction, eyeup, up_direction, angle_direction, left_direction, upleft, upright, lowleft, lowright;
	vector<vec3> frame;
	...
	for (int i = 0; i < data2.size(); ++i) {
		if (data2[i][0] == "V") {
			view_direction.set(stof(data2[i][1]), stof(data2[i][2]), stof(data2[i][3]));
	...
	left_direction = (view_direction ^ eyeup);
	left_direction = view_direction.length() * tan(angle * 3.1415926 / 180) * (view_direction ^ eyeup);
	vec3 use = left_direction;
	use.normalize();
	up_direction = view_direction.length() * tan(angle * 3.1415926 / 180) * (use ^ view_direction);



	int focal_length = 20;
	view_direction *= focal_length ;
	left_direction *= focal_length ;
	up_direction *= focal_length ;
	upleft = (eye + view_direction + left_direction + up_direction);
	upright = (eye + view_direction - left_direction + up_direction);
	lowleft = (eye + view_direction + left_direction - up_direction);
	lowright = (eye + view_direction - left_direction - up_direction);
